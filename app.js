...
function routeFromHash() {
  ...
  if (section === "internal") return showInternalPage(target || "reportsPage", extra || null);
...
function showInternalPage(pageId, extra = null) {
  ...
  if (pageId === "internalHomePage") setActiveNav("#/internal/reportsPage");
...
function updatePermissionVisibility() {
  ...
  $$(".nav-officer").forEach((el) => {
    const isInternalNav = el.classList.contains("nav-internal");
    el.classList.toggle("hidden", !isOfficerPlus() || (isInternalNav && !internalVisible));
  });

  $$(".sergeant-only").forEach((el) => el.classList.toggle("hidden", !isSergeantPlus()));
  $$(".admin-only").forEach((el) => el.classList.toggle("hidden", !isAdmin()));
  $$(".owner-only").forEach((el) => el.classList.toggle("hidden", !isOwner()));
  $$(".avk-only").forEach((el) => el.classList.toggle("hidden", !isAVK()));
}
